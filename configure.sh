...
read -p "GitHub ssh key created, please enter your GitHub password: " githubpassword
curl -u "$githubuser:$githubpassword" --data "{\"title\":\"GitHub_$(date +%Y%m%d%H%M%S)\",\"key\":\"$(cat ~/.ssh/id_rsa.pub)\"}" https://api.github.com/user/keys
unset githubpassword
